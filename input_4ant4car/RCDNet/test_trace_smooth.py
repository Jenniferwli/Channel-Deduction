...
plt.plot(range(len_trace-past_len)[::5],nmse_list[::5],linewidth=2,color='orange',
         linestyle='-',label='RCDNet (Working under error propagation)')
plt.plot(range(len_trace-past_len)[::5],nmse_list_truth[::5],linewidth=2,color='olive',
         linestyle='-',label='RCDNet (Working under the ideal case of no error propagation)')
plt.xlabel('Time slots')
plt.legend()
plt.grid(alpha=0.4)
plt.tight_layout()
plt.savefig('nmse_trace_RCDNet.jpg')
plt.savefig('nmse_trace_RCDNet.eps')
plt.savefig('nmse_trace_RCDNet.pdf')
plt.show()