...
class pred_net(nn.Module):
    def __init__(self,ant_size,car_size,depth_pred):
        ...
        # 1.Input Linear Layer: Maps the channel data to the LSTM's hidden dimension
        ...
        # 3.Output Linear Layer: Maps the LSTM's output from the hidden dimension back to the channel data dimension
...
class CDNet(nn.Module):
    def __init__(self,input_ant_size,input_car_size,ant_size,car_size,depth,depth_pred):
        ...
        # 2.use LSTM for channel prediction
        self.pred_net=pred_net(ant_size,car_size,depth_pred)
        # 3.use CMixer to further refine and optimize the LSTM's output 
